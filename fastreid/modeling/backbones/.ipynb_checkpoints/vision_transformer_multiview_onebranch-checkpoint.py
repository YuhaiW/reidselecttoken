...
from fastreid.layers.differential_topk import VisualTokenSelection
...
class VisionTransformer_multiview_onebranch(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, embed_dim=768,
        ...
        self.visual_token_selector = VisualTokenSelection(1, embed_dim, topk=2)
        # self.token_selectors = nn.ModuleList([VisualTokenSelection(1, embed_dim, topk=2) for _ in range(len(self.blocks))])
    ...
    def forward(self, x, camera_id=None): # [128, 3, 256, 128]
        B = x.shape[0]
        ...
        x = torch.cat((cls_tokens, view_tokens, x), dim=1) # [128, 128, 768] -> [128, 130, 768]

        if self.cam_num > 0:
            x = x + self.pos_embed + self.sie_xishu * self.sie_embed[camera_id]
        else:
            x = x + self.pos_embed # run this branch

        x = self.pos_drop(x)

        for blk in self.blocks: # 12 blocks
            x = blk(x) # [128, 130, 768] -> [128, 130, 768]
            # perform inner sub
            if self.inner_sub:
                x[:, 0] = x[:, 0] - x[:, 1]

#         for idx, blk in enumerate(self.blocks):  # 12 blocks
#             x = blk(x)  # [128, 130, 768] -> [128, 130, 768]
            
#             # if last 2 block, excute visual_token_selector
#             if idx == len(self.blocks) - 3:  
#                 x = self.visual_token_selector(x)
#             # token_selector = self.token_selectors[idx]  # Access the specific token selector for this block
#             # x = token_selector(x)
...
#                 x[:, 0] = x[:, 0] - x[:, 1]
        ...
        # token selection X torch.Size([128, 130, 768])
        # import pdb; pdb.set_trace()
        # x = self.visual_token_selector(x)### x.shape  torch.Size([128, 4, 768])
        

        x = self.norm(x) #torch.Size([128, 4, 768])

        return x[:, 0].reshape(x.shape[0], -1, 1, 1), x[:, 1].reshape(x.shape[0], -1, 1, 1) # cls_token [128, 768, 1, 1], view_token [128, 768, 1, 1]
